...
print(sys.platform) # Mostra qual plataforma voce esta utilizando ex: windows, linux, mac

cmd = ['ping', '127.0.0.1'] # Comando para saber o ping do pc
# cmd = ['ls -lah /']
encoding = 'cp850' # Codificação aceita pelo windows para escritas com acento, assim como utf-8 que não é aceito 
sistem = sys.platform

if sistem == 'darwin': # Se a plataforma for igual a do mac vai ser dado esses comandos no bloco de codigo
    cmd = ['ping', '127.0.0.1', '-c', '4']
    encoding = 'utf_8'

# Função que roda o codigo que vc quiser com cada parametro
proc = subprocess.run(
    cmd, capture_output=True,
    text=True, encoding=encoding # essa linha sao os argumentos q precisam para poder fazer o codigo sair no formato utf-8
)
